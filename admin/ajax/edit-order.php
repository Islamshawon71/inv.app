...
function update()
{
    ...
    $orderStatus = $_REQUEST['data']['orderStatus'];
    $MemoNumber = $_REQUEST['data']['MemoNumber'];
    $OrderDate = !empty($_REQUEST['data']['OrderDate']) ? $_REQUEST['data']['OrderDate'] : 'null';
    $DeliveryDate = !empty($_REQUEST['data']['DeliveryDate']) ? $_REQUEST['data']['DeliveryDate'] : 'null';
    $PaidReturnDate = !empty($_REQUEST['data']['PaidReturnDate']) ? $_REQUEST['data']['PaidReturnDate'] : 'null';
    ...
    $order = "UPDATE orders SET  total='" . $total . "', DeliveryCharge='" . $DeliveryCharge . "', DiscountCharge='" . $DiscountCharge . "', PaymentAmount='" . $PaymentAmount . "', OrderDate='" . $OrderDate . "' , DeliveryDate=" . $DeliveryDate . ", MemoNumber='" . $MemoNumber . "', PaidReturnDate=" . $PaidReturnDate . " , storeID='" . $storeID . "', status= '" . $orderStatus . "' WHERE orderID='" . $orderID . "'  ";
    mysqli_query($con, $order);


    $orderDetails = "UPDATE orderDetails SET CustomerName='" . $CustomerName . "',CustomerPhone='" . $CustomerPhone . "',CustomerAddress='" . $CustomerAddress . "',courierID='" . $courierID . "',cityID='" . $cityID . "',zoneID='" . $zoneID . "',PaymentType='" . $PaymentType . "',PaymentNumber='" . $PaymentNumber . "',PaymentAgentNumber='" . $PaymentAgentNumber . "' WHERE orderID='" . $orderID . "'  ";
    $orderDetails = mysqli_query($con, $orderDetails);

    $deleteProduct = "delete from orderProduct WHERE orderID='" . $orderID . "'  ";
...
function oldOrders()
{
    ...
    $sql = "SELECT orders.*,
            ...
            LEFT JOIN users on (orders.userID = users.userID) where orderDetails.CustomerPhone like '" . $CustomerPhone . "' and orders.orderID != '" . $orderID . "'";

    $results = mysqli_query($con, $sql);
    $temp = array();
    while ($result = mysqli_fetch_assoc($results)) {

        $products = "select * from orderProduct where orderID='" . $orderID . "'";
